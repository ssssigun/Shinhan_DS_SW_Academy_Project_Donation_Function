...
const StoreMain = () => {
  ...
  useEffect(() => {
    ...
    //메뉴 리스트 가져오기
    axios
      .get(`/db/selectMenuAndCount?store_pk=${location.state.storePk}`)
      .then((response) => {
        // 성공 처리
    ...
    >
          ...
          {menuData.map((menu) => (
            <li className="StoreMainMenu" onClick={() => move(menu, storeData)}>
              <div className="StoreMainMenuTextArea">
                ...
                <p className="StoreMainMenuTextPrice"></p>
              </div>
      ...
      <div className="StoreMainNoticeWrapper">
        ...
        <ul className="StoreMainNoticeText">
          <li className="StoreMainNotice">
            <p>메뉴 사진은 연출된 이미지로 실제 조리된 음식과 다를 수 있습니다.</p>
          </li>
          <li className="StoreMainNotice">
            <p>상단 메뉴 및 가격은 업소에서 제공한 정보를 기준으로 작성되었으며 변동될 수 있습니다.</p>
          </li>
          <li className="StoreMainNotice">
            <p>
              땡겨요는 상품거래에 대한 통신판매중개자이며, 통신판매의 당사자가 아닙니다. 따라서, 땡겨요는 상품 거래정보
